...
Axios のリクエスト／レスポンスインターセプターに `LoadingGameStore` の通知処理を組み込み、5 秒超過リクエストを検出する。

### Canvas ゲームエンジン（ローディングゲーム）

| API / 手法 | 用途 |
|-----------|------|
| **HTML Canvas 2D API** | 恐竜・サボテン・背景の描画 |
| **requestAnimationFrame** | 60fps ゲームループ |
| **カスタムシングルトン（`LoadingGameStore`）** | axios とReact を疎結合でつなぐ pub/sub ストア |

外部ライブラリを使用せず、ブラウザ標準 API のみで実装。`LoadingGameStore` はモジュールレベルのシングルトンとして動作し、axios インターセプターから通知を受け取った React クライアントコンポーネント（`LoadingGameProvider`）がモーダル表示を制御する。
...
|------|------|
| 2026-06-15 | 初版作成 |
| 2026-07-01 | Canvas ゲームエンジンセクション追加（ローディングゲーム実装に伴う技術追記） |
