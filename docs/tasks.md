...
**完了条件**
- [ ] 有効な JWT を付与したリクエストが `@UseGuards(JwtAuthGuard)` を通過する
- [ ] 無効な JWT では 401 が返る
- [ ] `@CurrentUser()` で `User` オブジェクトが取得できる
...
**完了条件**
- [ ] `month` クエリパラメータが `YYYY-MM` 形式以外のとき 400 が返る
- [ ] 他のユーザーのイベントを `PATCH`/`DELETE` すると 403 が返る
- [ ] クロス月イベントが正しく取得できる
