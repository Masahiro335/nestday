...
- [x] パターン作成時に `sortOrder` 未指定でも既存最大値+1 が自動付与され、重複しない
- [x] ▲/▼ での並び替えで一番上・一番下への移動が正しく動作する
...
- [x] レスポンスの `date` フィールドが `YYYY-MM-DD` 形式で返却される（ISO datetime ではなく）
...
- [x] グループ未所属で `/` にアクセスすると `/onboarding` にリダイレクトされる
...
**完了条件**
- [x] 正常なメール・パスワードでログインして `/` または `/onboarding` に遷移する
- [x] 新規登録後に `/onboarding` へ遷移する
- [x] エラー時にフォーム下部にエラーメッセージが表示される
...
**完了条件**
- [x] グループ作成後に招待リンクがコピーできる
- [x] 無効なトークンで `/join/[token]` にアクセスするとエラーが表示される
- [x] すでにグループ所属済みでの参加試行にエラーが表示される
...
**完了条件**
- [x] 月のイベントが正しく表示される
- [x] 日付をタップするとドロワーが開いてその日のイベント一覧が表示される
- [x] 月移動（前月・次月）が機能する
- [x] FAB タップで `/events/new` に遷移する
...
**完了条件**
- [x] イベントを作成して `/` に戻るとカレンダーにイベントが表示される
- [x] 作成者以外が編集画面にアクセスすると `/` にリダイレクトされる
- [x] 削除確認後にイベントが削除される
...
**完了条件**
- [x] グループ全員のシフトが月表示で確認できる
- [x] 自分の日付をタップするとシフト選択パネルが開く
- [x] パターンを選択するとシフトが登録・更新される
...
**完了条件**
- [x] パターン一覧が `sort_order` 順に表示される
- [x] `is_day_off: true` のパターンで時間入力欄が非表示になる
- [x] 勤務時間が「終了 - 開始 - 休憩」で自動計算される
- [x] 行全体を掴んで上下にドラッグし並び替えができる（PC・スマートフォン対応）
- [x] 短いクリック・タップで編集画面への遷移が機能する（ドラッグと共存）
...
| 2026-06-19 | T10: date フィールドの YYYY-MM-DD フォーマット返却を完了条件に追加 |
| 2026-06-19 | T09: sortOrder 自動付与と並び替え一括再割り当ての完了条件を追加 |
| 2026-06-19 | T20: ドラッグ＆ドロップ並び替えの完了条件を追加 |
| 2026-06-19 | T20: ドラッグ範囲を行全体に変更（クリックとの共存を完了条件に追加） |
